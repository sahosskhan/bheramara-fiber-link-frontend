...
import { Wifi, Router, Server, Zap } from "lucide-react"; // icons

const GetConnection = () => {
  const devices = [
    {
      name: "WiFi Router",
      desc: "High-speed dual band WiFi Router provided for client side.",
      icon: <Router size={40} className="text-pink-400" />,
    },
    {
      name: "XPON / GPON ONU",
      desc: "ONU device setup to connect with Fiber network.",
      icon: <Server size={40} className="text-blue-400" />,
    },
  ];

  const documents = ["1 Passport Size Photo",
    "1 NID Card Photocopy",
    "Full Name",
    "Contact Number",];
  const policies = ["Installation, setup & cable are free of cost.",
  ...
  return (
    <div className="bg-gradient-to-br from-[#0e1026] via-[#1f0b3a] to-[#0b253e] text-gray-100 min-h-screen p-6">
      ...
      <section className="max-w-6xl mx-auto ">
        <h2 className="text-4xl font-bold bg-teal-200 p-2 text-black rounded-xl text-center mb-14 animate-fadeIn delay-300">
          📌Client Side Requirement
        </h2>
        <div className="grid sm:grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn delay-400">
          {devices.map((device, i) => (
            ...
            >
              <div className="mb-4 flex justify-center">{device.icon}</div>
              <h3 className="text-2xl text-center font-bold mb-2 text-white">{device.name}</h3>
              <p className="text-gray-300 text-lg text-center">{device.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Documents */} 
      <section className="max-w-6xl mx-auto mt-12 bg-white/10 backdrop-blur-xl
 rounded-3xl p-8 shadow-lg animate-fadeIn delay-1000">
        <h2 className="text-4xl font-bold mb-12 text-center text-teal-500">Required Documents</h2>
        <ul className="grid sm:grid-cols-1 md:grid-cols-2 gap-4">
          {documents.map((doc, i) => (
            <li key={i} className="bg-white/10 p-4 rounded-2xl text-lg text-gray-200 shadow-md float-card" > 📜{doc} 
            </li>
          ))} 
          </ul>
           </section>
      ...
      {/* Policies */}
       <section className="max-w-6xl mx-auto mt-12  bg-white/10 backdrop-blur-xl 
       rounded-3xl p-8 shadow-lg animate-fadeIn delay-1200">
         <h2 className="text-4xl font-bold text-teal-500 mb-12 text-center">
          Our Policies
          </h2> 
          <ul className="space-y-3"> 
            {policies.map((pol, i) => (
              <li key={i} className="bg-white/5 p-3 text-lg rounded-xl float-card"> 
              🔹 {pol} </li>))} 
              </ul> 
              </section>
